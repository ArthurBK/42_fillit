...
int			ft_backtrack(t_coord **pieces, int size, char **res, int i_piece)
{
  ...
  while (i < size * size)
  {
      x_index = i % size;
      y_index = i / size;
    if (ft_check_htg(&pieces[i_piece], res, x_index, y_index, size) != 1 && (pieces[i_piece])->in == 0)
    {
//      if ((ft_check_htg_all(&pieces[i_piece], res, x_index, y_index, size) == 0) &&
  //      i_piece < pieces[0]->nb_pieces)
    //    return (ft_backtrack(pieces, size, res, i_piece + 1));
